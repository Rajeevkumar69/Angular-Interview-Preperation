...
export class NestedFormValidator {
     ...
     public singleError(group: FormGroup, formErrors: any = {}, validationMessage: any = {}): any {
          Object.keys(group.controls).forEach((key: string) => {
               ...
               if (abstractControl instanceof FormGroup) {
                    formErrors[key] = this.singleError(
                         abstractControl,
                         formErrors[key] || {},
                    ...
                    if (!abstractControl.valid && (abstractControl.touched || abstractControl.dirty)) {
                         if (validationMessage[key]) {
                              ...
                              for (const errorKey in abstractControl.errors) {
                                   if (
                                        abstractControl.errors.hasOwnProperty(errorKey) &&
                                        messages[errorKey]
                                   ) {
     ...
     public displayNestedFormErrors(
          group: FormGroup,
          key: string,
          errors: any = {},
          message: any = {},
          ...
          if (_tempGroup) {
                    ...
                    if (floorUnitArray && floorUnitArray.controls) {
                         floorUnitArray.controls.forEach((floorUnitControl, index) => {
                              if (floorUnitControl instanceof FormGroup) {
                                   ...
                                   floorUnitErrors[index].Floor = this.singleError(
                                        floorUnitControl,
                                        floorUnitErrors[index].Floor || {},
                                        message[key]?.Floor || {}
                                   );
                                   floorUnitErrors[index].UnitNo = this.singleError(
                                        floorUnitControl,
                                        floorUnitErrors[index].UnitNo || {},
                                        message[key]?.UnitNo || {}
                                   );
               ...
               key === 'lessorDetails' ||
               key === 'dateAndTimeLine' ||
               key === 'rentalAndSecurityDetails' ||
               key === 'MiscellaneousInformation'
          ) {
               const _tempGroup = group.get(key) as FormGroup;
               if (_tempGroup) {
                    if (isSingle) {
